    ...
    if (model.name.toLowerCase().includes('google')) {
      ...
      if (agent.mode === 'CHAT' || agent.mode === 'FILE') {
        if (files && files.length > 0) {
          ...
          if (hasExcel && modelRecord.expectedOutput === 'excel') {
            result = await this.geminiService.analyzeFilesChunked(
              files, finalPrompt, agent.systemPrompt, sendProgress
            );
          } else {
        ...
        if (files && files.length > 0) {
          ...
          res.write(`data: ${JSON.stringify({ type: 'error', message: `${agent.mode} mode processing requires files.` })}\n\n`);
          return res.end();
        }
      } else {
        res.write(`data: ${JSON.stringify({ type: 'error', message: 'Invalid agent mode' })}\n\n`);
        return res.end();
      }
      ...
      if (modelRecord.expectedOutput === 'excel' && result.text) {
        ...
        if (!validation.isValid) {
          result.message = `Error de Validación Contable: ${validation.error}. Por favor, revisa el prompt o los datos de entrada e intenta de nuevo.`;
        }
      }
    } else {
      return res.status(400).json({
        message: "Model currently not integrated or model name does not include 'google'",
      });
    }
